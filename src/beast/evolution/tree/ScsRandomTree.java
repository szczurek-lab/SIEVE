...
public class ScsRandomTree extends ScsTree implements StateNodeInitialiser {

    final public Input<ScsAlignment> taxaInput = new Input<>("taxa", "set of taxa to initialise " +
            "tree specified by alignment");
    ...
    final public Input<Double> rootHeightInput = new Input<>("rootHeight", "If specified the tree " +
            "will be scaled to match the root height, if constraints allow this");
    ...
    List<ScsRandomTree.Bound> m_bounds;
    ...
    public void initAndValidate() {
        ...
        if (taxaInput.get() != null) {
            // if data is specified, get taxa information
            taxa.addAll(taxaInput.get().getTaxaNames());
        } else {
            // if data is not specified, get taxa information from tree input
...
//	}
        ...
        for (final MRCAPrior prior : calibrations) {
            ...
            if (taxonSet != null && !prior.onlyUseTipsInput.get()) {
                ...
                final ScsRandomTree.Bound bounds = new ScsRandomTree.Bound();
                if (distr != null) {
        ...
        nodeCount = 2 * taxa.size();
        internalNodeCount = taxa.size();
    ...
    public void simulateTree(final Set<String> taxa, final PopulationFunction demoFunction) {
        ...
        for (int attempts = 0; attempts < 1000; ++attempts) {
            try {
                ...
                /*
                 * ScsTree class is a normal coalescent tree with an extra root node and a trunk
                 * so simulateCoalescent() function should return rootTMRCA node
                 */
                rootTMRCA = simulateCoalescent(lastMonophyletic, allCandidates, candidates, demoFunction);

                // get the final tree with trunk
                root = simulateTreeWithTrunk(rootTMRCA);

                return;
            } catch (RandomTree.ConstraintViolatedException | ConstraintViolatedException e) {
    ...
    private Node simulateCoalescent(final int isMonophyleticNode, final Map<String, Node> allCandidates, final Set<Node> candidates, final PopulationFunction demoFunction)
            throws RandomTree.ConstraintViolatedException, ConstraintViolatedException {
        ...
        return simulateCoalescentWithMax(remainingCandidates, demoFunction, upper);
    }
    ...
    public Node simulateCoalescentWithMax(final List<Node> nodes, final PopulationFunction demographic,
                                          final double maxHeight) throws RandomTree.ConstraintViolatedException, ConstraintViolatedException {
    ...
    public List<Node> simulateCoalescent(final List<Node> nodes, final PopulationFunction demographic, double currentHeight,
                                         final double maxHeight) throws ScsRandomTree.ConstraintViolatedException {
     ...
     */
    @SuppressWarnings("deprecation")
    private double coalesceTwoActiveNodes(final double minHeight, double height) throws ScsRandomTree.ConstraintViolatedException {
...
//			}
            ...
            if (max < min) {
                ...
                throw new ScsRandomTree.ConstraintViolatedException();
            }
    ...
    /**
     * simulate the final tree on the basis of a tumor coalescent tree with an extra tree root node and trunk
     *
     * @param tumorRoot the root of the simulated coalescent tree
     * @return root of the final tree
     */
    @SuppressWarnings("deprecation")
    public Node simulateTreeWithTrunk(Node tumorRoot) {
        Node treeRoot = newNode();
        double tumorRootHeight = tumorRoot.getHeight();
        ...
        if (expectedNrOfMuInput.get() != null) {
            // if expectedNrOfMuInput is defined
            // inverse cumulative probability function of exponential distribution with mean being expectedNrOfMuInput
            trunkLength = -expectedNrOfMuInput.get() * Math.log(Randomizer.nextDouble());
        } else {
            // if expectedNrOfMuInput is not defined
            // inverse cumulative probability function of exponential distribution parameterized by current population size
            trunkLength = -Math.log(Randomizer.nextDouble()) / populationFunctionInput.get().getPopSize(tumorRootHeight);
        }

        rootHeight = tumorRootHeight + trunkLength;

        treeRoot.setNr(nextNodeNr++);
        treeRoot.setHeight(rootHeight);
        treeRoot.setLeft(tumorRoot);
        tumorRoot.setParent(treeRoot);

        return treeRoot;
    } // simulateTreeWithTrunk
