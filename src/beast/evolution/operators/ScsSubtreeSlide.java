...
@Description("Modified based on SubtreeSlide move")
public class ScsSubtreeSlide extends TreeOperator {
    ...
    public double proposal() {
        ...
        // 1. choose a random node avoiding root and sample root
        ...
        if (delta > 0) {
            ...
            // 3.0 is it a valid move?
            if (newHeight >= tree.getRoot().getHeight()) {
                return Double.NEGATIVE_INFINITY;
            }

            // 3.1 if the topology will change
            if (!PiP.isRoot() && PiP.getHeight() < newHeight) {
                // find new parent
                Node newParent = PiP;
                Node newChild = p;
                ...
                // 3.1.2 count the hypothetical sources of this destination.
